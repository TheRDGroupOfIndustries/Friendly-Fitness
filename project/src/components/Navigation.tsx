      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4">
        <div className="flex justify-between items-center py-3 sm:py-4">
          <div className="flex items-center space-x-2 sm:space-x-3 text-xl sm:text-2xl font-bold min-w-0">
            <img src={logo} alt="Fitness Revolution Logo" className="h-9 w-9 sm:h-10 sm:w-10 rounded-full object-cover flex-shrink-0" />
            <span className="text-white truncate">Fitness </span>
            <span className="text-orange-500 truncate">Revolution</span>
          </div>
              ...
              <NavLink
                ...
                className={({ isActive }: { isActive: boolean }) =>
                  `text-sm font-medium uppercase px-2 py-1 text-white hover:text-[#F97316] transition-colors focus:outline-none ${
                    isActive ? 'text-[#F97316] border-b-2 border-[#F97316] pb-1' : ''
                  }`
                }
                end={item.to === '/'}
              >
        ...
        <div className={`fixed inset-0 z-40 transition-all duration-300 ${isMenuOpen ? 'block' : 'pointer-events-none'}`}
        ...
        >
          ...
          />
          {/* Slide-down menu */}
          <div className={`absolute top-0 left-0 w-full backdrop-blur-md shadow-lg transition-transform duration-300 ${isMenuOpen ? 'translate-y-0' : '-translate-y-full'} md:hidden pb-4 pt-2`} style={{ backgroundColor: '#090E26' }}
            role="menu"
          >
