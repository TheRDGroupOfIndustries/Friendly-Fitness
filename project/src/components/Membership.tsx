...
import { client } from "../sanityClient"; // make sure sanityClient is configured
...
const Membership: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        const data: Plan[] = await client.fetch(
            `*[_type == "membership"] | order(_createdAt asc) {
        _id,
        name,
        price,
        features,
        ...
        if (data && data.length > 0) {
          ...
          setPlans(hardcodedPlans); // fallback if no plans in Sanity
        }
      } catch (error) {
        console.error("Error fetching membership plans:", error);
        setPlans(hardcodedPlans); // fallback on error
      }
    };
    ...
    <section className="py-20" style={{ backgroundColor: "#090E26" }}>
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-5xl md:text-7xl font-bold text-white mb-4" id="title">
          ...
          {plans.map((plan, idx) => (
            <div
              ...
              className={`rounded-2xl shadow-lg text-white bg-blue-950 p-4 sm:p-8 flex flex-col items-center border-2 border-gray-200 transition-transform duration-300 hover:scale-105 ${
                plan.highlight ? "border-yellow-400" : ""
              }`}
            >
